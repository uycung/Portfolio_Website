...
const WaveBackground = ({
  lineColor = "rgb(37, 64, 74)", // Keep existing, demo uses "black" or "#fff"
  backgroundColor = "rgb(0, 0, 0)", // Keep existing, demo uses "transparent" or "rgba(255, 255, 255, 0.2)"
  waveSpeedX = 0.0125, // Demo: 0.0125 or 0.02
  waveSpeedY = 0.005,  // Demo: 0.005 or 0.01
  waveAmpX = 32,       // Demo: 32 or 40
  waveAmpY = 16,       // Demo: 16 or 20
  xGap = 10,           // Demo: 10 or 12
  yGap = 32,           // Demo: 32 or 36
  friction = 0.925,    // Demo: 0.925 or 0.9
  tension = 0.005,     // Demo: 0.005 or 0.01
  maxCursorMove = 100, // Demo: 100 or 120
  style = {},
  className = ""
}) => {
  ...
  useEffect(() => {
    ...
    function drawLines() {
      ...
      linesRef.current.forEach((points) => {
        ...
        ctx.moveTo(p1.x, p1.y); // Added from demo for smoother line start
        points.forEach((p, idx) => {
          const isLast = idx === points.length - 1;
          p1 = moved(p, !isLast);
          const p2 = moved(points[idx + 1] || points[points.length - 1], !isLast);
          ctx.lineTo(p1.x, p1.y);
          if (isLast) ctx.moveTo(p2.x, p2.y); // This line seems redundant if the loop continues correctly, but matches demo
        });
      });
      ctx.stroke();
    }
  ...
  return (
    <div
      ...
      className={`fixed top-0 left-0 w-full h-full overflow-hidden z-0 ${className}`}
    >
      {/* Removed the mouse follower div as it's not in the primary demo code and might affect performance or visual smoothness */}
      {/* 
      <div
